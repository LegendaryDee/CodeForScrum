...
public class Main {
 ...
 private static void handleTimScenario()
 {
    ...
    timUser = new User(UUID.randomUUID(), "ttom", "password123", "tim@example.com", LanguagePreference.ENGLISH, new ProgressData(UUID.randomUUID()), 0);
...
private static void showUserProgress(User user) {
    ...
    private static void reviewStrugglingContent(User tammyUser) {
        ...
        List<String> strugglingPhrases = List.of("Buenos dias", "Buenas noches", "Hasta luego", "Lo siento", "�C�mo est�s?", "Estoy bien", "Perd�n", "Muchas gracias");
    ...
    private static void registerNewUser(Scanner scanner) {
        ...
        User newUser = new User(userID, userName, password, email, languagePreference, new ProgressData(userID), 0);
